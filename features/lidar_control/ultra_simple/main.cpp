...
#include <../../third_party/rplidar_sdk/sdk/include/sl_lidar.h>
#include <third_party/rplidar_sdk/sdk/include/sl_lidar_driver.h>
...
bool checkSLAMTECLIDARHealth(sl::ILidarDriver * drv) {
    ...
    if (SL_IS_OK(op_result)) {
        // the macro IS_OK is the preperred way to judge
        ...
        if (healthinfo.status == SL_LIDAR_STATUS_ERROR) {
            ...
            // enable the following code if you want slamtec lidar
            // to be reboot by software
            // drv->reset();
            return false;
        } else {
            return true;
...
int main(int argc, const char * argv[]) {
    ...
    if (strcmp(opt_is_channel, "--channel") == 0) {
        opt_channel = argv[2];
        if (strcmp(opt_channel, "-s") == 0 || strcmp(opt_channel,
                    "--serial") == 0) {
        ...
        } else if (strcmp(opt_channel, "-u") == 0 || strcmp(opt_channel,
                    "--udp") == 0) {
            // read ip addr from the command line...
            opt_channel_param_first = argv[3];
            // or set to a fixed value: e.g. "192.168.11.2"
            if (argc > 4) opt_channel_param_second =
                strtoul(argv[4], NULL, 10);  // e.g. "8089"
            opt_channel_type = sl::CHANNEL_TYPE_UDP;
        } else {
            print_usage(argc, argv);
            return -1;
        }
    ...
    if (opt_channel_type == sl::CHANNEL_TYPE_SERIALPORT) {
        if (useArgcBaudrate) {
            _channel = (*sl::createSerialPortChannel(opt_channel_param_first,
            ...
            for (size_t i = 0; i < baudRateArraySize; ++i) {
                _channel = (*sl::createSerialPortChannel(
                            opt_channel_param_first, baudrateArray[i]));
                if (SL_IS_OK((drv)->connect(_channel))) {
                    op_result = drv->getDeviceInfo(devinfo);
    ...
    } else if (opt_channel_type == sl::CHANNEL_TYPE_UDP) {
        _channel = *sl::createUdpChannel(opt_channel_param_first,
                opt_channel_param_second);
        if (SL_IS_OK((drv)->connect(_channel))) {
            op_result = drv->getDeviceInfo(devinfo);
    ...
    if (!connectSuccess) {
        (opt_channel_type == sl::CHANNEL_TYPE_SERIALPORT)?
            (fprintf(stderr, "Error, cannot bind to the specified serial port"
                     "%s.\n", opt_channel_param_first)):(fprintf(stderr,
                         "Error, cannot connect to the specified ip addr %s.\n"
    ...
    drv->startScan(0, 1);
    // Message Queue top
    mq = mq_open("/T_lidar", O_WRONLY, 0666, &attr);
    // Mess
    if (mq == -1) {
    perror("open error");
    }
            ...
            for (int pos = 0; pos < static_cast<int>(count); ++pos) {
                if (nodes[pos].dist_mm_q2/4.0f > 0 &&
                    ...
                    if (pre_angle == NULL) {
                        ...
                        ob[num_obstacle].end_angle = nodes[pos].angle_z_q14;
                        pre_angle = nodes[pos].angle_z_q14;
                    } else {
                        num_obstacle += 1;
                        ob[num_obstacle].start_angle = nodes[pos].angle_z_q14;
    ...
    for (int P = 0; P <= num_obstacle; P++) {
        ...
        if (P == 0) {
            std::string tmp = std::to_string(num_obstacle);
