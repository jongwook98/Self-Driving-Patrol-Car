...
int main(int argc, const char * argv[]) {
    ...
    while (1) {
	    ...
	    obstacle * ob = new obstacle[10];
		    ...
		    if (nodes[pos].dist_mm_q2/4.0f > 0 && nodes[pos].dist_mm_q2/4.0f < 1000){
			    ...
			    else if (pre_angle - nodes[pos].angle_z_q14 <= 1000 && pre_angle - nodes[pos].angle_z_q14 >= - 1000){
	    ...
	    if (ob[0].start_angle <= 1000 && ob[num_obstacle].end_angle >= 64625){
	...
	for (int P = 0; P <= num_obstacle; P++){
		...
		char num[5];
		char info[35];
		sprintf(info, ",angle : %06.2f dis : %07.2f " , ob[P].min_angle*90.f/16384.f, ob[P].min_dis/4.0f);
		sprintf(num, "%d", P);

		if (P == 0){
			string tmp = to_string(num_obstacle);
		...
		//printf("obs : %d, min_angle : %03.2f, min_dis : %08.2f \n", P, ob[P].min_angle*90.f/16384.f, ob[P].min_dis/4.0f);
		}
	delete []ob;
	}
	//Message
