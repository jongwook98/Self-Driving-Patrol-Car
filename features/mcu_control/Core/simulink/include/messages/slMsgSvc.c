...
void _slMsgCopyDataFromMsg(slMsgManager *mgr, void *buf, slMessage *msg, size_t bufsize)
{
    ...
    for (i = 0; (i < mgr->fNumPools) && (meminpool == 0); ++i) {
        pool = &(mgr->fPoolMgr.fPools[i]);
        meminpool = __slmsg_MemPoolForData(pool, msg->fData);
    }
    if (meminpool && _slMsgIsDataPointerPool(pool)) {
        _slMsgDataPointerDeepCopy(pool, buf, msg->fData);
    }
    else if (meminpool && _slMsgNeedDataHandling(pool)) {
