...
{
    // htim->Instance->ARR 		// counter period(auto-reload register)
    // set set htim->Instance->psc 		// prescaler set
...
{
    // htim->Instance->ARR 		// counter period(auto-reload register)
    // set htim->Instance->psc 		// prescaler set
    ...
    TIM8->CNT = 0;
    control_flow.inputs->Input2.motor_dir_u16 =
    ...
    HAL_GPIO_WritePin(break_light_GPIO_Port, break_light_Pin,
                      !control_flow.outputs->Output1.start_stop_u8);

    HAL_GPIO_WritePin(
        motor_dir_GPIO_Port, motor_dir_Pin,
        control_flow.outputs->Output1.motor_dir_u8); // motor _direction
