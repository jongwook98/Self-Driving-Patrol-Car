...
void main_while(void)
{
    ...
    // TxPrintf("|sample : %d|value : %d|\n", g_motor.u16qep_sample,
    // g_motor.int16qep_value ); TxPrintf("|sample : %d|value : %d|\n",
    // g_motor.u16qep_sample, HAL_GPIO_ReadPin(PB7_MOTOR_DIR_GPIO_Port,
    // PB7_MOTOR_DIR_Pin) ); TxPrintf("flag1 : %u |flag2 : %u pid_out : %f |\n",
    // (PD7_LED_GPIO_Port->IDR &
    // PD7_LED_Pin),HAL_GPIO_ReadPin(PD7_LED_GPIO_Port, PD7_LED_Pin),
    // g_motor.fp32PID_output );

    // PA12_MOTOR_DIR_GPIO_Port->BSRR = PA12_MOTOR_DIR_Pin;  // gpio set;
    // PA12_MOTOR_DIR_GPIO_Port->BSRR = (uint32_t)PA12_MOTOR_DIR_Pin << 16U;
    // // gpio reset
