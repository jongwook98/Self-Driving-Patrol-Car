...
void USARTx_TxString(volatile UART_HandleTypeDef *USARTx, uint8_t *buffer,
                     char *Str)
{
    uint16_t strCnt = 0;
    while (*Str)
...
void linux_protocol(uint8_t *buffer, uint8_t *_buf)
{
    if (*_buf == 0x55 && *(_buf + 4) == 0x77)
    {
        ...
        control_flow.inputs->Input1.input_angle_r32 =
            (real_T)st_ptcl.angle - 90.0;
    ...
    /*
            ...
            if(*_buf == 'w' || *(buffer + 4) != '\0') {
                    ...
                    sscanf((const char*)buffer,"%c%c%c%c%c", &curr.start,
                            &curr.mode, &curr.flag.all, &curr.angle,
       &curr.stop);
                    ...
                    if(curr.start == 0x55 || curr.stop == 0x77);
                            ...
                            TxPrintf("%x %x %x %x %x\n", st_ptcl.start,
       st_ptcl.mode, st_ptcl.flag.all, st_ptcl.angle, st_ptcl.stop);

                            control_flow.inputs->Input1.input_angle_r64 =
       (real_T)st_ptcl.angle - 90.0; control_flow.inputs->flag.blinker 	=
       st_ptcl.flag.bit.blinker; control_flow.inputs->flag.turn_move =
       st_ptcl.flag.bit.turn_onoff;
                            control_flow.inputs->flag.move 		=
       st_ptcl.flag.bit.move_onoff;
                    }
                    else
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
    ...
    if (huart->Instance == USART1)
    {
        ...
        case 'e':
            ...
            {
                control_flow.inputs->flag.steer =
                    control_flow.inputs->flag.turn = 0;
            }
            break;
        case 'r':
